...
- **A connection is inspectable in both views:** a story arc pins open to show what it carries, what comes back, and the files at each end; a flow route opens to list the bindings that cross it. Aggregation carries the union of crossing bindings up from the file-level edges it merges, bounded, because a route whose only label is its weight says two modules touch without saying why.
