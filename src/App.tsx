...
const LAST_REPOSITORY_KEY = "codebase-atlas:last-repository";
...
function App() {
  ...
  async function scanPath(path: string) {
    ...
    try {
      ...
      setRailOpen(false);
      localStorage.setItem(LAST_REPOSITORY_KEY, path);
    } catch (scanError) {
  ...
  useEffect(() => {
    ...
    const savedPath = localStorage.getItem(LAST_REPOSITORY_KEY);
    if (savedPath) {
      void scanPath(savedPath);
    ...
    <div className="app-shell">
      ...
      <header className="instrument-bar">
        ...
        <div className="instrument-readings" aria-label="Repository summary">
          ...
          <div className="instrument-reading metric-reading">
            <span>Lines</span>
            <strong>{graph ? graph.stats.lines.toLocaleString() : "--"}</strong>
          </div>
      ...
      <div className="workspace">
        ...
        >
          ...
          <div className="module-groups">
            ...
            {!graph ? <p className="rail-empty">Scan a directory to populate the module index.</p> : null}
          </div>
        </nav>
        ...
        <main id="repository-map" className="map-panel" tabIndex={-1}>
          ...
          {graph ? (
            ...
            <section className="empty-state" aria-labelledby="empty-title">
              ...
              <p>
                Select a local repository to inspect its structure, languages, and modules as a navigable technical field.
              </p>
              <button
                type="button"
                onClick={() => void chooseRepository()}
              ...
              >
                {isTauriRuntime ? "Choose repository" : "Open in desktop app"}
              </button>
              {!isTauriRuntime ? (
                <small>Browser preview is interface-only. Directory access remains inside Tauri.</small>
              ) : null}
            </section>
          )}
          ...
          {loading ? (
            <div className="loading-plate" role="status" aria-live="polite">
              ...
              <span>Reading structure, languages, and line counts…</span>
            </div>
          ) : null}
              ...
              {inspectorTab === "overview" ? (
                ...
                >
                  ...
                  <section>
                    ...
                    <div className="signal-grid">
                      <div>
                        <strong>{selectedNode.lines.toLocaleString()}</strong>
                        <span>Lines</span>
                      </div>
                      <div>
                      ...
                      <ol className="language-register">
                        {graph.stats.languages.slice(0, 8).map((language) => (
                          <li key={language.name}>
                            ...
                            <b>{language.lines.toLocaleString()}</b>
                          </li>
                        ))}
                      </ol>
                    </section>
                ...
                >
                  <dl className="detail-list full-details">
                    ...
                    <div>
                      <dt>Lines</dt>
                      <dd>{selectedNode.lines.toLocaleString()}</dd>
                    </div>
                    <div>
                      <dt>Bytes</dt>
      ...
      <footer className="status-strip">
        <div>
          ...
          {loading ? "Scanning" : graph ? "Map ready" : isTauriRuntime ? "Awaiting source" : "Browser preview"}
        </div>
        <p>
          Drag to orbit · secondary drag to pan · scroll to zoom · <kbd>/</kbd> search · <kbd>0</kbd> reset
        </p>
        <div className="status-path" title={graph?.root}>
          {graph ? `${visibleLayerCount}/4 layers · ${graph.root}` : "LOCAL / READ ONLY"}
        </div>
      </footer>
    </div>
  );
